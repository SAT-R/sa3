...
void Player_800522C(Player *p)
{
    ...
    p->moveState &= ~(MOVESTATE_20000000 | MOVESTATE_2000000 | MOVESTATE_1000000 | MOVESTATE_TAG_ACTION_CHARGED | MOVESTATE_200000
                      | MOVESTATE_80000 | MOVESTATE_40000 | MOVESTATE_GRINDING | MOVESTATE_8000 | MOVESTATE_4000 | MOVESTATE_2000
            ...
            if (!sub_8015064(p)) {
                ...
                if (!sub_8015064(p)) {
                    ...
                    if (((p->charFlags.someIndex == 2) && (p->charFlags.character == AMY))
                        && ((partner->charFlags.character == SONIC) && (p->charFlags.anim0 == 0))) {
...
void Player_8005E80(Player *p)
{
    u16 sp00[NUM_CHARACTERS][2] = {
        [SONIC] = { 0x268, 0x268 },    [CREAM] = { 0x27B, 0x27B }, [TAILS] = { 0x26C, 0x26D },
        [KNUCKLES] = { 0x271, 0x273 }, [AMY] = { 0x277, 0x277 },
    };
    s16 *temp_r0_2;
    s16 temp_r2;
    u8 *temp_r0;
...
void sub_80063B4(Player *p, s32 arg1)
{
    ...
    Player_PlayOrContinueSong(p, SE_JUMP);
...
void Player_8007DF4(Player *p)
{
    ...
    if (MOVESTATE_2000000 & partner->moveState) {
        sub_800BF78(partner);
        p->moveState &= ~(MOVESTATE_1000000 | MOVESTATE_TAG_ACTION_CHARGED);
        Player_StopSong(p, SE_TAG_ACTION_CHARGING);
        return;
    }
    if (!(partner->moveState & MOVESTATE_TAG_ACTION_CHARGED)) {
...
void Player_8008080(Player *p)
{
    ...
    if (((charAnim == 2) || (charAnim == 5)) && (p->charFlags.state1 == 0)) {
        ...
        } else if (p->moveState & MOVESTATE_GRAVITY_SWITCHED) {
...
NONMATCH("asm/non_matching/game/stage/player__Player_8008A8C.inc", void Player_8008A8C(Player *p))
{
    ...
    if (++p->idleAndCamCounter > 0x1F) {
        ...
        if (temp_r0_2 & MOVESTATE_TAG_ACTION_CHARGED) {
            sub_800E150(p);
            return;
        }
        p->callback = sub_800DF10;
...
void Player_8008CD0(Player *p)
{
    ...
    p->moveState |= MOVESTATE_IN_AIR;
    p->moveState &= ~(MOVESTATE_20000000 | MOVESTATE_IGNORE_INPUT | MOVESTATE_COLLIDING_ENT | MOVESTATE_2);
    sub_8012FA0(p);
...
{
    u16 sp00[NUM_CHARACTERS] = { [SONIC] = VOICE__SONIC__WHOA,
                                 [CREAM] = VOICE__CREAM__OH,
                                 [TAILS] = VOICE__TAILS__UARGH,
                                 [KNUCKLES] = VOICE__KNUCKLES__HUH,
                                 [AMY] = VOICE__AMY__NO };
    Player *partner;
...
{
    u16 charDeadSongIDs[NUM_CHARACTERS] = { [SONIC] = VOICE__SONIC__WHOA,
                                            [CREAM] = VOICE__CREAM__OH,
                                            [TAILS] = VOICE__TAILS__UARGH,
                                            [KNUCKLES] = VOICE__KNUCKLES__HUH,
                                            [AMY] = VOICE__AMY__NO };
...
#define SUB_800913C_PSET(_player)                                                                                                          \
    ...
    (_player)->timeSinceTagActionDelay = 0;                                                                                                \
    (_player)->timeSinceTagActionCharge = 0;                                                                                               \
    (_player)->unk44 = 0;                                                                                                                  \
    ...
    (_player)->unk5A = 0;                                                                                                                  \
    (_player)->unk5B = 0;                                                                                                                  \
    (_player)->timerSpeedup = 0;                                                                                                           \
    (_player)->timerSlowItem = 0;                                                                                                          \
    (_player)->unk62 = 0;                                                                                                                  \
    (_player)->unk64 = 0;                                                                                                                  \
    (_player)->unk66 = 0;                                                                                                                  \
    (_player)->unk68 = 0;
...
{
    u16 sp00[NUM_CHARACTERS] = { 168, 184, 204, 230, 254 };
...
void Player_80098D8(Player *p)
{
    ...
    p->unk9A = 0;
    p->qCamOffsetY = 0;
    p->moveState = (p->moveState & ~6) | MOVESTATE_GRINDING;
    sub_8012FA0(p);

    partner = GET_SP_PLAYER_V1(PLAYER_2);
    if ((p->charFlags.character == SONIC) && (partner->charFlags.character == AMY)) {
...
void sub_80099FC(Player *p)
{
    ...
    if (p->moveState & MOVESTATE_IN_AIR) {
        ...
        if (sub_801480C(p) != 0) {
            p->moveState &= ~MOVESTATE_GRINDING;
            Player_StopSong(p, SE_GRINDING);
    ...
    sub_8014BC4(p);
    if (MOVESTATE_GRINDING & p->moveState) {
        if (p->moveState & MOVESTATE_TAG_ACTION_CHARGED) {
            p->layer = PLAYER_LAYER_BACK;
            Player_StopSong(p, SE_GRINDING);
            p->moveState &= ~MOVESTATE_GRINDING;
...
void Player_8009B4C(Player *p)
{
    ...
    sub_8009D74(p);
    temp_r1 = p->moveState;
    if (temp_r1 & MOVESTATE_TAG_ACTION_CHARGED) {
        p->moveState = (temp_r1 & 0xFDFFFFFD) | MOVESTATE_TAG_ACTION_CHARGED;
        p->charFlags.anim0 = 0x15;
        sub_8012FA0(p);
        temp_r2 = p->spriteInfoBody;
        temp_r2->s.frameFlags &= ~0x4000;
...
void Player_8009BF0(Player *p)
{
    sub_8009D74(p);
    if (p->moveState & MOVESTATE_TAG_ACTION_CHARGED) {
        p->moveState = (p->moveState & 0xFDFFFFFD) | MOVESTATE_TAG_ACTION_CHARGED;
        p->charFlags.anim0 = 0x15;
        sub_8012FA0(p);
        p->spriteInfoBody->s.frameFlags &= ~0x4000;
        p->callback = sub_800E198;
...
void Player_8009E8C(Player *p)
{
    ...
    if (p->moveState & MOVESTATE_TAG_ACTION_CHARGED) {
        p->moveState &= ~(MOVESTATE_2000000 | MOVESTATE_2);
        p->moveState |= MOVESTATE_TAG_ACTION_CHARGED;
...
void Player_8009F7C(Player *p, s16 someX, s16 someY)
{
    ...
    temp_r1_2 = *temp_r5;
    temp_r1_2->s.frameFlags &= ~0x4000;
    p->qSpeedAirX += someX;
    p->qSpeedAirY = someY - Q(4.875);
    Player_PlaySong(p, 0x114U);
    temp_r1_3 = p->moveState;
    if (temp_r1_3 & MOVESTATE_TAG_ACTION_CHARGED) {
        p->moveState = (temp_r1_3 & 0xFDFFFFFD) | MOVESTATE_TAG_ACTION_CHARGED;
        p->charFlags.anim0 = 0x15;
        sub_8012FA0(p);
        temp_r1_4 = *temp_r5;
        temp_r1_4->s.frameFlags &= ~0x4000;
        p->callback = sub_800E198;
        sub_800E198(p);
        return;
    }
...
void Player_800A094(Player *p)
{
    ...
    p->unk9A = 0;
    p->qCamOffsetY = 0;
    p->moveState = (p->moveState | MOVESTATE_JUMPING) & ~MOVESTATE_10000000;
    sub_8012FC0(p);
    p->charFlags.anim0 = 0x13;
    temp_r2 = p->spriteInfoBody;
    temp_r2->s.frameFlags &= ~0x4000;
...
void Player_800A3C0(Player *p)
{
    ...
    sub_8012FA0(p);
    p->moveState = (p->moveState & ~(MOVESTATE_2000000 | MOVESTATE_8 | MOVESTATE_2)) | MOVESTATE_20000000;
    sub_8016F28(p);
...
void Player_800B1B8(Player *p)
{
    ...
    gPlayers[p->charFlags.partnerIndex].moveState &= ~MOVESTATE_2000000;
    p->charFlags.unk2D_0 = 0;
    p->charFlags.unk2C_04 = 0;
    p->qSpeedGround = 0;
    p->qSpeedAirX = 0;
...
void Player_800B628(Player *p)
{
    ...
    if (temp_r0_2 < temp_r5) {
        p->qWorldX = temp_r0_2 + Q(2);
    }
    temp_r0_3 = p->qWorldY;
    if (temp_r0_3 > temp_r6) {
        p->qWorldY = temp_r0_3 - Q(2);
    }
    temp_r0_4 = p->qWorldY;
    if (temp_r0_4 < temp_r6) {
        p->qWorldY = temp_r0_4 + Q(2);
    }
...
void Player_800B748(Player *p)
{
    ...
    if (--p->idleAndCamCounter == 0) {
        ...
        p->callback = Player_800DAF4;
        temp_r0_4 = (0xDDFFFFBF & p->moveState) | MOVESTATE_IN_AIR;
        p->moveState = temp_r0_4;
        if (temp_r0_4 & MOVESTATE_TAG_ACTION_CHARGED) {
            sub_800E150(p);
            return;
        }
        p->callback = sub_800DF10;
...
void sub_800BF78(Player *p)
{
    ...
    temp_r0->moveState = (temp_r0->moveState & 0xFFFEFFFF) | (p->moveState & MOVESTATE_GRAVITY_SWITCHED);
    if (p->moveState & 4) {
#ifndef NON_MATCHING
...
void sub_800C1FC(Player *p)
{
    ...
    if (sub_8012E18(1, p, 0, 0) < 0) {
        ...
        p->moveState = temp_r0;
        if (temp_r0 & MOVESTATE_TAG_ACTION_CHARGED) {
            sub_800E150(p);
        } else {
            p->callback = sub_800DF10;
            sub_800DF10(p);
...
void sub_800C5D0(Player *p)
{
    ...
    p->moveState = (p->moveState | 6) & ~MOVESTATE_20000000;
    Player_8012FF0(p);
    var_r1 = p->qSpeedAirX;
    if (var_r1 < 0) {
        var_r1 = -var_r1;
    ...
    if (!sub_8014BC4(p)) {
        if ((p->charFlags.state1 == 0) && (p->qSpeedAirY >= 0)) {
            ...
            if (0x4000 & spriteSprite->s.frameFlags) {
                spriteSprite->s.frameFlags &= ~0x4000;
                p->charFlags.state1 = 2;
            }
        }
...
void sub_800C9C4(Player *p)
{
    ...
    p->moveState = temp_r1;
    p->moveState = temp_r1 | (partner->moveState & MOVESTATE_GRAVITY_SWITCHED);
    if (sub_8015064(p)) {
        return;
    }
    sub_8011D08(p);
    ...
    if ((p->charFlags.anim0 != 0x5C) || (callback = p->callback, (callback != sub_800C9C4))
        ...
        do {
            partner->moveState &= ~MOVESTATE_2000000;
        } while (0);
        p->callback(p);
        return;
    }
...
void sub_800CBE8(Player *p)
{
    ...
    p->moveState |= gPlayers[p->charFlags.partnerIndex].moveState & MOVESTATE_GRAVITY_SWITCHED;

    if (SPRITE_FLAG_GET(&p->spriteInfoBody->s, ANIM_OVER)) {
...
block_43:
    ...
    block_44:
        ...
        if (temp_r0_3 & MOVESTATE_TAG_ACTION_CHARGED) {
            sub_800E150(p);
            return;
        }
        p->callback = sub_800DF10;
...
{
    p->moveState = (p->moveState & 0xFDFFFFBF) | MOVESTATE_TAG_ACTION_CHARGED;
...
{
    p->moveState = (p->moveState | 0x02000000) & ~MOVESTATE_20000000;
    sub_8012FA0(p);
    p->idleAndCamCounter = 360;
    if (p->qSpeedGround != 0) {
        sub_800E27C(p);
...
void sub_800E764(Player *p)
{
    ...
    if (--p->idleAndCamCounter == 0) {
        temp_r0_2 = p->moveState & ~MOVESTATE_20000000;
        p->moveState = temp_r0_2;
        if (temp_r0_2 & 4) {
            p->callback = Player_800DB30;
        } else {
...
void Player_800E7BC(Player *p)
{
    p->moveState = (p->moveState & ~0x40) | 0x22000004;
    p->charFlags.anim0 = 0x5C;
    p->qSpeedGround = 0;
    p->qSpeedAirX = 0;
    p->qSpeedAirY = 0;
    p->unk26 = 0;
    Player_8012FE0(p);
    Player_PlaySong(p, SE_TAILS__FLYING);
    p->callback = sub_800C9C4;
    sub_800C9C4(p);
}

void Player_800E808(Player *p)
{
    if ((s32)p->qSpeedAirY >= 0) {
        if (p->charFlags.state1 == 1) {
            p->charFlags.state1++;
...
void sub_800E8E0(Player *p)
{
    ...
    temp_r3->s.frameFlags &= ~0x4000;
    p->idleAndCamCounter = 0xA;
    p->qSpeedGround = 0;
    p->qSpeedAirX = 0;
    p->qSpeedAirY = 0;
    p->callback = sub_800D238;
    sub_800D238(p);
}
...
// TODO: anim0 indices
// TODO: Maybe gUnknown_080CE5FC and gUnknown_080CE60C are one 2D array?
const s16 gUnknown_080CE60C[8] = {
    0x11A, 0x11F, 0x11D, 0x121, 0x11B, 0x120, 0x11C, 0x11E,
};

const s16 gUnknown_080CE61C[8][2] = {
    { 0x0800, 0x0000 }, //
    { 0x0599, 0x0599 }, //
    { 0x0000, 0x0800 }, //
    { 0xFA67, 0x0599 }, //
...
void sub_80106E0(Player *p)
{
    ...
    temp_r5 = p->unk148.ptr;
    if (gCamera.unk6A != 0) {
        var_r0 = p->qWorldX + Q(gCamera.unk6A) + Q(4.5);
        var_r1 = 0;
...
void sub_80107B4(Player *p)
{
    ...
    qPrevWorldX = p->qWorldX;
    temp_r3 = qPrevWorldX - Q(1);
    p->qWorldX -= Q(1);

    if (gCamera.unk6A != 0) {
        p->qWorldX = temp_r3 + Q(gCamera.unk6A) + Q(4.5);
    } else {
        if (gCamera.x == 0) {
            p->qWorldX = qPrevWorldX - Q(4.5);
        }
        p->qWorldX = p->qWorldX + Q(gCamera.unk6A);
    }
...
s32 sub_8011024(s16 arg0, Player *p, u8 *arg2, u32 *arg3)
{
    u8 dummyArg2;
    u32 dummyArg3;
    s32 temp_r2;
    ...
    switch (arg0) {
        ...
        case 2:
            sp14 = SA2_LABEL(sub_801E4E4)(I(spC) - p->spriteOffsetY, +I(sp10) - 2 - p->spriteOffsetX, temp_r1, -8, &p->charFlags.unk28,
                                          SA2_LABEL(sub_801ED24));
            var_r2 = SA2_LABEL(sub_801E4E4)(I(spC) - p->spriteOffsetY, +I(sp10) + 2 + p->spriteOffsetX, temp_r1, -8, &p->charFlags.unk29,
                                            SA2_LABEL(sub_801ED24));
            break;
        case 3:
            sp14 = SA2_LABEL(sub_801E4E4)(I(spC) + p->spriteOffsetY, +2 + I(sp10) + p->spriteOffsetX, temp_r1, +8, &p->charFlags.unk28,
                                          SA2_LABEL(sub_801ED24));
            var_r2 = SA2_LABEL(sub_801E4E4)(I(spC) + p->spriteOffsetY, -2 - p->spriteOffsetX + I(sp10), temp_r1, +8, &p->charFlags.unk29,
                                            SA2_LABEL(sub_801ED24));
            break;
    }
...
NONMATCH("asm/non_matching/game/stage/player__sub_8011BFC.inc", s32 sub_8011BFC(u8 arg0, Player *p))
{
    ...
    switch (arg0 >> 6) {
        ...
        case 1: {
            s32 p0 = -2 + worldX;
            return SA2_LABEL(sub_801E4E4)(p0 - p->spriteOffsetX, worldY, p->layer, -8, NULL, SA2_LABEL(sub_801ED24));
        } break;
...
void sub_8011D08(Player *p)
{
    ...
    temp_r0 = SA2_LABEL(sub_801E4E4)(worldX, worldY, var_r2, -8, NULL, SA2_LABEL(sub_801ED24));
    var_r2_4 = temp_r0;
    if (var_r2_4 <= 0) {
    ...
    temp_r0_2 = SA2_LABEL(sub_801E4E4)(var_r3, worldY, (s32)var_r2_2, 8, NULL, SA2_LABEL(sub_801ED24));
    var_r2_4 = temp_r0_2;
    if (var_r2_4 <= 0) {
        p->qWorldX += Q(var_r2_4);
        p->qSpeedAirX = 0;
...
NONMATCH("asm/non_matching/game/stage/player__sub_8011E70.inc", void sub_8011E70(Player *p))
{
    ...
    temp_r0_2 = SA2_LABEL(sub_801E4E4)(worldX, worldY, var_r2_2, 8, NULL, SA2_LABEL(sub_801ED24));
    var_r2_n = temp_r0_2;
    if (var_r2_n <= 0) {
...
NONMATCH("asm/non_matching/game/stage/player__sub_8011FB8.inc", void sub_8011FB8(Player *p))
{
    ...
    if (p->moveState & MOVESTATE_GRAVITY_SWITCHED) {
        ...
        var_r1 = I(p->qWorldY);
        var_r2 = temp_r0;
        temp_r0_2 = SA2_LABEL(sub_801E4E4)(var_r0, var_r1, var_r2, -8, NULL, SA2_LABEL(sub_801ED24));
    } else {
        var_r0 = -3 - p->spriteOffsetX + I(p->qWorldX);
        var_r1 = I(p->qWorldY);
        var_r2 = temp_r0;
...
NONMATCH("asm/non_matching/game/stage/player__sub_801226C.inc", s32 sub_801226C(Player *p))
{
    ...
    temp_r0_2 = SA2_LABEL(sub_801E4E4)(worldX2, worldY2, var_r2_2, 8, NULL, SA2_LABEL(sub_801ED24));
    if (temp_r0_2 <= 0) {
...
NONMATCH("asm/non_matching/game/stage/player__sub_8012368.inc", s32 sub_8012368(Player *p))
{
    ...
    temp_r0 = SA2_LABEL(sub_801E4E4)(worldX2, worldY2, var_r2_2, 8, NULL, SA2_LABEL(sub_801ED24));
    if (temp_r0 <= 0) {
        p->qWorldX += Q(temp_r0);
    }
...
void sub_80126B8(Player *p)
{
    ...
    temp_r0 = SA2_LABEL(sub_801E4E4)(worldX, worldY, (s32)var_r2, -8, NULL, SA2_LABEL(sub_801ED24));
    off = temp_r0;
    if (off <= 0) {
        temp_r0 = Q(off);
        p->qWorldX -= temp_r0;
    ...
    temp_r0 = SA2_LABEL(sub_801E4E4)(worldX3, worldY3, var_r2, 8, NULL, SA2_LABEL(sub_801ED24));
    off = temp_r0;
    if (off <= 0) {
        temp_r0 = Q(off);
        p->qWorldX += temp_r0;
...
void sub_8012930(Player *p)
{
    ...
    temp_r0 = SA2_LABEL(sub_801E4E4)(worldX, worldY, unk27, -8, NULL, SA2_LABEL(sub_801ED24));
    res = temp_r0;
    if (res <= 0) {
        temp_r0 = Q(res);
        p->qWorldX -= temp_r0;
...
void sub_8012A6C(Player *p)
{
    ...
    temp_r0 = SA2_LABEL(sub_801E4E4)(worldX2, worldY, unk27, +8, NULL, SA2_LABEL(sub_801ED24));
    res = temp_r0;
    if (res <= 0) {
        temp_r0 = Q(res);
        p->qWorldX += temp_r0;
...
s32 sub_8012E18(s32 arg0, Player *p, u8 *arg2, u32 *arg3)
{
    s32 result;
    u16 var_r2;
    ...
    if (var_r2 == 0) {
        result = sub_8011024(2, p, arg2, arg3);
    } else {
        result = sub_8011024(3, p, arg2, arg3);
    }
    ...
    if (p->charFlags.anim0 != -1) {
        ...
        switch (temp_r0_3) {
            ...
            case 9:
                ...
                break;
            default:
                if ((p->charFlags.anim0 < 0xA5) && (p->charFlags.anim0 != -1)) {
...
void sub_80142CC(Player *p)
{
    ...
    if (gStageData.gameMode != 7) {
        ...
        p->unk90 = gUnknown_080CECB2[ringSpeedFactor][0];
        p->unk94 = gUnknown_080CECB2[ringSpeedFactor][1];
...
void Player_8014550(Player *p)
{
    ...
    if (!(gStageData.unk4 == 5 || gStageData.unk4 == 6) && ((p->framesInvulnerable <= 0)) && (p->framesInvincible <= 0)) {
        if (p->moveState & MOVESTATE_GRINDING) {
            p->layer = PLAYER_LAYER_BACK;
            Player_StopSong(p, SE_GRINDING);
            p->moveState &= ~MOVESTATE_GRINDING;
        }
...
void Player_80149E4(Player *p)
{
    ...
    var_r1 = (p->moveState & MOVESTATE_GRAVITY_SWITCHED) ? 0x80 : 0;
    temp_r0 = p->qSpeedAirX;
...
bool16 sub_8014A60(Player *p)
{
    if ((DPAD_ANY & p->keyInput) == DPAD_DOWN) {
        if ((p->qSpeedGround == 0) && !((p->unk26 + 0x20) & 0xC0) && !(p->moveState & (MOVESTATE_GRINDING | MOVESTATE_JUMPING))) {
            SetPlayerCallback(p, Player_8007FE8);
            return TRUE;
        }

        if (p->qSpeedGround <= -Q(0.5) || p->qSpeedGround >= +Q(0.5)) {
            if (!((MOVESTATE_GRINDING | MOVESTATE_JUMPING) & p->moveState) && ((p->unkC & 6) == 2)
                && !(p->moveState & MOVESTATE_TAG_ACTION_CHARGED)) {
                SetPlayerCallback(p, Player_800891C);
                Player_PlaySong(p, SE_SPIN_ATTACK);
...
s16 sub_8014BC4(Player *p)
{
    ...
    if (p->keyInput & gStageData.buttonConfig.trick) {
        ...
        if (p->timeSinceTagActionDelay < TAG_ACTION_DELAY) {
            p->timeSinceTagActionDelay++;
        } else {
            if (!(p->moveState & MOVESTATE_TAG_ACTION_CHARGING)) {
            ...
            if (p->timeSinceTagActionCharge == TAG_ACTION_CHARGING_TIME) {
                Player_BoostModeDisengage(p);
                sub_80193A4(p);

                if ((p->moveState & (MOVESTATE_COLLIDING_ENT | MOVESTATE_IN_AIR)) == MOVESTATE_IN_AIR) {
                ...
                p->moveState &= ~(MOVESTATE_TAG_ACTION_CHARGING | MOVESTATE_8 | MOVESTATE_2);
                p->moveState |= MOVESTATE_TAG_ACTION_CHARGED;
                p->timeSinceTagActionCharge = 0;
                return TRUE;
            }
        ...
        if ((p->unkC & 0x800000) && (p->charFlags.unk2C_04) && (p->timeSinceTagActionDelay != 0)
            && (p->timeSinceTagActionDelay < TAG_ACTION_DELAY)) {
            p->charFlags.unk2C_04 = 0;
            p->timeSinceTagActionDelay = 0;
            sub_800C3AC(p);
        } else {
            ...
            if (p->moveState & MOVESTATE_TAG_ACTION_CHARGED) {
                Player *partner = GET_SP_PLAYER_V1(PLAYER_2);
                sub_800BF78(p);
                partner->moveState &= ~(MOVESTATE_TAG_ACTION_CHARGED | MOVESTATE_1000000);
                p->moveState &= ~(MOVESTATE_TAG_ACTION_CHARGED | MOVESTATE_1000000);
                sub_80193CC(p);
                Player_StopSong(p, SE_TAG_ACTION_CHARGING);
            } else {
                if (p->timeSinceTagActionCharge != 0) {
                    Player_StopSong(p, SE_TAG_ACTION_CHARGING);
                }
                sub_80193A4(p);
                p->moveState &= ~MOVESTATE_TAG_ACTION_CHARGING;
...
void sub_8014E70(Player *p)
{
    ...
    for (i = 0; i < 2; i++) {
        ...
        } else if (!(p->moveState & MOVESTATE_FACING_LEFT)) {
            var_r2 = -0x40;
        }
...
void sub_8016F28(Player *p)
{
    ...
    temp_r1 = p->moveState;
    if (MOVESTATE_TAG_ACTION_CHARGED & temp_r1) {
...
void Task_80191C8(void)
{
    ...
    if (p->timerSlowItem == 0 && p->unk62 == 0) {
        p->unkDC = NULL;
        TaskDestroy(gCurTask);
        return;
    }
    ...
    if ((gStageData.zone != 8) || (p == &gPlayers[PLAYER_1])) {
        if (gStageData.taskAfterImages == 0) {
            ...
            if (gStageData.zone != ZONE_UNUSED) {
                ...
                RetrieveInitialAnimationPalette(ANIM_PALETTE_SUPER_SONIC_AFTER_IMAGE, 0x10);
            }
...
void Task_AfterImages(void)
{
    ...
    if (0x20 & s->frameFlags) {
        ...
        if (subStrc->moveState & MOVESTATE_GRAVITY_SWITCHED) {
...
{
    if (!(p->moveState & MOVESTATE_80) && (p->unk14C.arr_s16[0] != 0)) {
...
void sub_801C1D4(Player *p)
{
    ...
    if ((p->moveState & MOVESTATE_80) && (p->unk14C.arr_s16[0] != 0)) {
        ...
        if (0x200000 & temp_r1) {
            SetPlayerCallback(p, sub_801B8DC);
            p->charFlags.anim0 = 0xD6;
        } else if (temp_r1 & MOVESTATE_TAG_ACTION_CHARGING) {
            SetPlayerCallback(p, sub_801BA2C);
            p->charFlags.anim0 = 0xD9;
        }
        Player_StopSong(p, SE_TAILS__FLYING);
...
void sub_801C4A0(Player *p)
{
    ...
    if ((s8)p->unk148.arr_u8[1] >= 0) {
        ...
        if ((DPAD_UP & p->keyInput) && (p->charFlags.anim2 == 0x23C)) {
            p->moveState = p->moveState & ~MOVESTATE_20000000;
            ...
            if (var_r4 > 0) {
                ...
                p->moveState = var_r0 & ~MOVESTATE_20000000;
        ...
        Player_8012FE0(p);
        p->qWorldY += 0xFFFFFB00;
        p->moveState = (p->moveState | MOVESTATE_IN_AIR) & ~MOVESTATE_20000000;
        SetPlayerCallback(p, sub_801D9D0);
        sub_801D9D0(p);
    }
}
...
s32 sub_801C884(Player *p)
{
    ...
    if (p->moveState & 1) {
        ...
        res = SA2_LABEL(sub_801E4E4)(qWorldX - p->spriteOffsetX, I(p->qWorldY), p->layer, -8, &rotation, SA2_LABEL(sub_801ED24));
        if (1 & rotation) {
            p->unk26 = +0x40;
        } else {
            p->unk26 = rotation;
...
void sub_801CE94(Player *p)
{
    ...
    var_r3 = 0;
    if (p->keyInput2 & gStageData.buttonConfig.jump) {
        var_r3 = (u32)(0 - (p->moveState & 0x02000000)) >> 0x1F;
    }
    if (!(p->keyInput & gStageData.buttonConfig.jump) && !(p->moveState & 0x02000000)) {
        var_r3 = 1;
    }
    if (var_r3 != 0) {
        if (p->qSpeedAirX <= 0) {
    ...
    sub_801CF4C(p);
    if (p->moveState & 0x02000000) {
        temp_r5->qWorldX = p->qWorldX + p->qSpeedAirX;
        temp_r5->qWorldY = p->qWorldY + p->qSpeedAirY;
    }
}
...
void sub_801CFA4(Player *p)
{
    ...
    if (!(var_r4 & 0x7F)) {
        ...
        p->charFlags.anim0 = 0xE8;
        p->charFlags.anim2 = 0x203;
        p->charFlags.state1 = (s16)((u32)(var_r4 & 0x7F) >> 5);
    }
}
...
void sub_801D3F0(Player *p)
{
    ...
    if ((s32)((p->unk148.arr_u8[0] + 0x40) << 0x18) <= 0) {
        p->moveState |= 1;
        temp_r0 = sub_8011024(0, p, NULL, &sp8);
        if (temp_r0 != sp8) {
            SetPlayerCallback(p, sub_801DF80);
            return;
        }
        ...
        temp_r0 = sub_8011024(1, p, NULL, &sp8);
        if (temp_r0 == sp8) {
            if (temp_r0 != 0) {
                if (!(p->moveState & MOVESTATE_GRAVITY_SWITCHED)) {
                    s32 qWorldX, qWorldY;
...
void sub_801D5CC(Player *p)
{
    ...
    if ((s16)p->idleAndCamCounter != 0) {
        if (--p->idleAndCamCounter == 0) {
            p->qSpeedAirY = -Q(1.5);
            Player_PlaySong(p, 0x70U);
        }
        sub_8017004(p);
        if ((0x24 & p->moveState) != 4) {
            p->moveState = p->moveState | 2;
            sub_8012FC0(p);
            p->charFlags.state1 = 1;
        ...
        if (!(0x100 & p->moveState) && ((0x24 & p->moveState) != 4)) {
            p->moveState = p->moveState | 2;
            sub_8012FC0(p);
            p->charFlags.state1 = 1;
            SetPlayerCallback(p, sub_801D684);
...
void sub_801DA04(Player *p)
{
    ...
    if ((sub_8015064(p) << 0x10) == 0) {
        ...
        sub_8012EB8(p);
        sub_8016E00(p);
        if (!(p->moveState & 0x100) && (p->spriteInfoBody->s.frameFlags & 0x4000)) {
            Player_PlaySong(p, 0x7EU);
            p->charFlags.state1 += 1;
...
void sub_801DB70(Player *p)
{
    ...
    if ((sub_8015064(p) << 0x10) == 0) {
        ...
        sub_8017004(p);
        if (!(p->moveState & 0x100)) {
            if ((p->callback == Player_8005380) || (p->callback == Player_80077CC)) {
                p->charFlags.state1 += 1;
...
void sub_801DC60(Player *p)
{
    ...
    var_r4 = 0;
    if (p->keyInput2 & gStageData.buttonConfig.jump) {
        var_r4 = (u32)(0 - (p->moveState & 0x02000000)) >> 0x1F;
    }
    if (!(p->keyInput & gStageData.buttonConfig.jump) && !(p->moveState & 0x02000000)) {
        var_r4 = 1;
    }
...
void sub_801DD68(Player *p)
{
    u8 sp;
    s32 var_r2;

    sub_8012634(p);
    var_r2 = sub_8012E18(1, p, &sp, NULL);
    if (var_r2 <= 0xB) {
        if (p->moveState & MOVESTATE_GRAVITY_SWITCHED) {
            var_r2 = 0 - var_r2;
        }
...
void sub_801DE1C(Player *p)
{
    PlayerSpriteInfo *temp_r2;
    s32 var_r0;
    u32 temp_r0;

    p->charFlags.anim0 = 0xF5;
    temp_r0 = p->moveState & ~MOVESTATE_20000000;
    p->moveState = temp_r0;
    if (temp_r0 & 0x10000) {
    ...
    p->spriteInfoBody->s.frameFlags &= ~0x4000;
    p->unk148.arr_u8[0] = 0;
    SetPlayerCallback(p, sub_801C934);
    sub_801C934(p);
}
...
{
    p->charFlags.anim0 = 0xF2;
    p->qSpeedGround = 0;
    p->qCamOffsetY = 0;
    p->idleAndCamCounter = 0x28;
    SetPlayerCallback(p, sub_801CC14);
    sub_801CC14(p);
}
...
void sub_801E0AC(Player *p)
{
    ...
    qDiff = Q(4);
    if (p->qSpeedAirX >= -Q(4) && p->qSpeedAirX <= +Q(4)) {
        if (p->moveState & 1) {
            p->qSpeedAirX = -Q(4);
        } else {
            p->qSpeedAirX = +Q(4);
...
void sub_801ECAC(Player *p)
{
    ...
    if ((refPlayer != p) && (p != NULL)) {
        ...
        if ((temp_r3->unk54 != NULL) && (temp_r3->unk54->charFlags.someIndex == 5)) {
            ...
            if (temp_r3->player->moveState & MOVESTATE_GRAVITY_SWITCHED) {
...
void Task_CheeseInit(void)
{
    ...
    cheese = TASK_DATA(gCurTask);
    cheese->qWorldX2 = (s32)(cheese->qWorldX + Q(24));
    cheese->qWorldY2 = (s32)(cheese->qWorldY - Q(24));
    cheese->unk10 = 0;
    cheese->unk12 = 0;
    cheese->unk14 = 0x100;
    cheese->moveState = 0;
    ...
    s->x = I(cheese->player->qWorldX);
    s->y = I(cheese->player->qWorldY);
    s->oamFlags = 0x540;
    s->qAnimDelay = 0;
    s->prevAnim = -1;
    ...
    s->hitboxes[0].index = -1;
    s->hitboxes[1].index = -1;
    gCurTask->main = Task_802056C;
}

void Task_801EE74(void)
{
    Cheese *cheese = TASK_DATA(gCurTask);
    Sprite2 *s;
    cheese->qWorldX2 = cheese->qWorldX;
    cheese->qWorldY2 = cheese->qWorldY;
    cheese->unk10 = 0;
    cheese->unk12 = 0;
    cheese->moveState = (u16)(0xFFF9 & cheese->moveState);
    cheese->unk18 = 0;
    cheese->unk19 = 0;
    cheese->unk1A = 0;
...
void sub_801EEE8(void)
{
    ...
    if (cheese->player->moveState & 1) {
        cheese->qWorldX2 = (s32)(cheese->player->qWorldX + Q(24));
    } else {
        cheese->qWorldX2 = (s32)(cheese->player->qWorldX - Q(24));
    }

    if (cheese->player->moveState & MOVESTATE_GRAVITY_SWITCHED) {
        cheese->qWorldY2 = (s32)(cheese->player->qWorldY + Q(24));
    } else {
        cheese->qWorldY2 = (s32)(cheese->player->qWorldY - Q(24));
    }

    if (cheese->player->qWorldX < (s32)cheese->qWorldX) {
        cheese->moveState |= 1;
    } else {
        cheese->moveState &= ~1;
    }
...
void sub_801EF6C(void)
{
    ...
    cheese->unk10 = 0;
    cheese->unk12 = 0;
    cheese->moveState = (u16)((0xFFFB & cheese->moveState) | 2);
    cheese->unk18 = 1;
    cheese->unk19 = 4;
    cheese->unk1A = 0;
...
void sub_801EFE8(void)
{
    ...
    cheese = TASK_DATA(gCurTask);
    cheese->unk1B = (cheese->unk1B + 1) % 64u;
    theta = cheese->unk1B;
    player = cheese->player;

    cheese->qWorldX2 = (player->qWorldX + ((v = COS(theta *= 16)) >> 1));
    cheese->qWorldY2 = (player->qWorldY + ((v2 = SIN(theta)) >> 1));
...
void Task_801F064(void)
{
    ...
    cheese->unk10 = 0;
    cheese->unk12 = 0;
    cheese->moveState = (u16)((0xFFFD & cheese->moveState) | 4);
    cheese->unk18 = 15;
    cheese->unk19 = 12;
    cheese->unk1A = 30;
...
void Task_801F0DC(void)
{
    ...
    if ((player->charFlags.character) != 1) {
        cheese->unk19 = temp_r6;
    } else {
        cheese->unk19 = 6;
    }
    cheese->moveState &= 0xFFFD;
    cheese->unk1A = 0x3C;
    s = &cheese->s;
...
void Task_801F184(void)
{
    ...
    sub_8020284();
    if (--cheese->unk1A == 0) {
        if (cheese->player->charFlags.character != CREAM) {
            gCurTask->main = Task_801FC2C;
        } else if (cheese->player->moveState & 0x100) {
            cheese->unk1A = 0x3C;
        } else if ((cheese->player->unkC & 0x18) == 0x10) {
...
void sub_801F258()
{
    ...
    frameFlags = 0;
    cheese->qWorldX2 = cheese->player->qWorldX;
    if (cheese->player->moveState & MOVESTATE_GRAVITY_SWITCHED) {
        cheese->qWorldY2 = cheese->player->qWorldY + Q(16);
    } else {
        cheese->qWorldY2 = cheese->player->qWorldY - Q(16);
    }
    cheese->unk1A = 0x18;
    ...
    s->anim = gCheeseTileInfo[0][0];
    s->variant = (u8)gCheeseTileInfo[0][1];
    s->qAnimDelay = 0;
    s->prevAnim = -1;
    s->prevVariant = -1;
    s->hitboxes[0].index = -1;
    s->hitboxes[1].index = -1;
    gCurTask->main = Task_801F2FC;
}
...
void Task_801F2FC()
{
    ...
    if (player->moveState & MOVESTATE_GRAVITY_SWITCHED) {
        cheese->qWorldY2 = player->qWorldY + Q(12);
    } else {
        cheese->qWorldY2 = player->qWorldY - Q(12);
    }
    if (cheese->player->moveState & 1) {
    ...
    if ((ABS(cheese->qWorldX2 - cheese->qWorldX) < (Q(4) - 1)) && (ABS(cheese->qWorldY2 - cheese->qWorldY) < (Q(5) - 1))) {
        cheese->s.anim = gCheeseTileInfo[7][0];
        cheese->s.variant = gCheeseTileInfo[7][1];
        gCurTask->main = Task_801F418;
    }
...
void Task_801F4B4(void)
{
    ...
    s = &cheese->s;
    s->frameFlags = 0x1000;
    s->anim = gCheeseTileInfo[9][0];
    s->variant = gCheeseTileInfo[9][1];
    s->qAnimDelay = 0;
    s->prevAnim = -1;
    s->prevVariant = -1;
    s->hitboxes[0].index = -1;
    s->hitboxes[1].index = -1;
    gCurTask->main = Task_8020660;
}
...
void Task_801F534()
{
    ...
    s = &cheese->s;
    s->frameFlags = 0x1000;
    s->anim = gCheeseTileInfo[10][0];
    s->variant = gCheeseTileInfo[10][1];
    s->qAnimDelay = 0;
    s->prevAnim = -1;
    s->prevVariant = -1;
    s->hitboxes[0].index = -1;
    s->hitboxes[1].index = -1;
    gCurTask->main = Task_80206B0;
}
...
void Task_801F5B4()
{
    ...
    cheese->qWorldX2 = (s32)player->unkB0;
    cheese->qWorldY2 = (s32)player->unkB4;
    cheese->moveState |= 2;
    cheese->unk18 = 9;
    cheese->unk19 = 2;
    cheese->unk1A = 0x1E;
    ...
    s = &cheese->s;
    s->anim = gCheeseTileInfo[2][0];
    s->variant = gCheeseTileInfo[2][1];
    s->qAnimDelay = 0;
    s->prevAnim = -1;
    s->prevVariant = -1;
    s->hitboxes[0].index = -1;
...
void Task_801F630()
{
    ...
    if (player->unkBC != -1U) {
        cheese->qWorldX2 = player->unkB0;
        cheese->qWorldY2 = player->unkB4;
    }
...
void Task_801F6D8()
{
    ...
    s = &cheese->s;
    s->anim = gCheeseTileInfo[3][0];
    s->variant = gCheeseTileInfo[3][1];
    s->qAnimDelay = 0;
    s->prevAnim = -1;
    s->prevVariant = -1;
    s->hitboxes[0].index = -1;
    s->hitboxes[1].index = -1;
    gCurTask->main = Task_801F740;
}
...
void Task_801F7E0(void)
{
    ...
    s->hitboxes[0].index = -1;
    s->hitboxes[1].index = -1;
    gCurTask->main = Task_801F848;
}

void Task_801F848(void)
{
    Cheese *cheese = TASK_DATA(gCurTask);
    Player *player;
    s32 v, v2;
...
void Task_801F8D8(void)
{
    ...
    if (player->moveState & 1) {
        cheese->qWorldX2 = player->qWorldX - Q(24);
    } else {
        cheese->qWorldX2 = player->qWorldX + Q(24);
    }

    cheese->qWorldY2 = cheese->player->qWorldY;
    cheese->moveState |= 2;
    cheese->unk18 = 9;
    cheese->unk19 = 8;
    cheese->unk1A = 0xA;
    ...
    temp_r1 = &cheese->s;
    temp_r1->anim = gCheeseTileInfo[8][0];
    temp_r1->variant = (u8)gCheeseTileInfo[8][1];
...
void Task_801FA64(void)
{
    ...
    cheese->unk1A = 0xA;
    s = &cheese->s;
    s->anim = gCheeseTileInfo[8][0];
    s->variant = (u8)gCheeseTileInfo[8][1];
    s->qAnimDelay = 0;
    s->prevAnim = -1;
    s->prevVariant = -1;
    s->hitboxes[0].index = -1;
...
void Task_801FAFC()
{
    ...
    if (--cheese->unk1A == 0) {
        cheese->qWorldX2 = (((COS(cheese->unk1D * 4)) * 2) / 5) >> 2;
        cheese->qWorldY2 = (((SIN(cheese->unk1D * 4)) * 2) / 5) >> 2;
        if (cheese->unk1C != 0) {
            cheese->qWorldX2 = -cheese->qWorldX2;
            cheese->qWorldY2 = -cheese->qWorldY2;
        }
        cheese->unk1A = 0xF;
        gCurTask->main = Task_801FBA8;
    }
}

void Task_801FBA8(void)
{
    Cheese *cheese = TASK_DATA(gCurTask);
    ...
    sub_8020130(0);
    cheese->unk1A -= 1;
    cheese->qWorldX = cheese->player->qWorldX + (cheese->qWorldX2 * (15 - cheese->unk1A));
    cheese->qWorldY = cheese->player->qWorldY + (cheese->qWorldY2 * (15 - cheese->unk1A));
    sub_8020284();
    if (cheese->unk1A == 0) {
        gCurTask->main = Task_801EE74;
    }
...
void Task_801FC2C(void)
{
    ...
    charIndex = (cheese->player->charFlags.character * 4 + 0xD);
    ...
    cheese->unk10 = 0;
    cheese->unk12 = 0;
    cheese->moveState = (0xFFFD & cheese->moveState) | 4;
    cheese->unk18 = 0;
...
void Task_8020038(void)
{
    ...
    if (cheese->player->qWorldX < cheese->qWorldX) {
        cheese->moveState = 1 | cheese->moveState;
    } else {
        cheese->moveState = 0xFFFE & cheese->moveState;
    }
...
void CreateCheeseIfPlayerIsCream(Player *p)
{
    Cheese *cheese;
...
void Task_80205F4()
{
    ...
    if (--cheese->unk1A == 0) {
        Player *player = cheese->player;
        struct Task *taskCheese = gStageData.taskCheese;

        if (player->charFlags.character == CREAM) {
            if ((player->unkC & 0x18) != 0x10) {
                taskCheese->main = Task_801EE74;
            } else {
                taskCheese->main = sub_801EF6C;
            }
        } else {
            taskCheese->main = Task_801FC2C;
        }
    }
...
s32 sub_80210BC(Player *playerA, s16 hbIndexA, Player *playerB, s16 hbIndexB)
{
    ...
    sprBodyB = &playerB->spriteInfoBody->s;
    sprBodyA = &playerA->spriteInfoBody->s;
    result = 0;

    if (playerB->moveState & 0x100) {
        return result;
    }
    if (sprBodyA->hitboxes[hbIndexA].index == -1) {
        return result;
    }

    if (sprBodyB->hitboxes[hbIndexB].index == -1) {
        return result;
    }
    ...
    if (gStageData.gameMode != 7) {
        if (hbIndexB == 0) {
            ...
            rectB[2] = +playerB->spriteOffsetX;
            rectB[3] = +playerB->spriteOffsetY;
        } else {
            rectB[0] = sprBodyB->hitboxes[hbIndexB].b.left;
            rectB[1] = sprBodyB->hitboxes[hbIndexB].b.top;
            rectB[2] = sprBodyB->hitboxes[hbIndexB].b.right;
            rectB[3] = sprBodyB->hitboxes[hbIndexB].b.bottom;
        }

        if (hbIndexA != 0) {
            rectA[0] = sprBodyA->hitboxes[hbIndexA].b.left;
            rectA[1] = sprBodyA->hitboxes[hbIndexA].b.top;
            rectA[2] = sprBodyA->hitboxes[hbIndexA].b.right;
            rectA[3] = sprBodyA->hitboxes[hbIndexA].b.bottom;
        } else {
            rectA[0] = -playerA->spriteOffsetX;
            rectA[1] = -playerA->spriteOffsetY;
            rectA[2] = +playerA->spriteOffsetX;
            rectA[3] = +playerA->spriteOffsetY;
        }
    } else {
        rectB[0] = -playerB->spriteOffsetX;
        rectB[1] = -playerB->spriteOffsetY;
    ...
    playerBWorldX = I(playerB->qWorldX);
    playerBWorldY = I(playerB->qWorldY);
    playerAWorldX = I(playerA->qWorldX);
    playerAWorldY = I(playerA->qWorldY);

    if (HB_COLLISION(playerAWorldX, playerAWorldY, ((Rect8) { rectA[0], rectA[1], rectA[2], rectA[3] }), playerBWorldX, playerBWorldY,
                     ((Rect8) { rectB[0], rectB[1], rectB[2], rectB[3] }))) {
        s32 middleX = playerAWorldX + ((rectA[0] + rectA[2]) >> 1);
        s32 middleY = playerAWorldY + ((rectA[1] + rectA[3]) >> 1);
        if ((middleX) <= playerBWorldX) {
            dx = (playerAWorldX + rectA[2]) - (playerBWorldX + rectB[0]);
            var_r0 = 0x40000;
        } else {
            dx = (playerAWorldX + rectA[0]) - (playerBWorldX + rectB[2]);
            var_r0 = 0x80000;
        }
        result |= var_r0;

        if (middleY > playerBWorldY) {
            dy = (rectA[1] + playerAWorldY) - (rectB[3] + playerBWorldY);
            if ((playerB->moveState & MOVESTATE_GRAVITY_SWITCHED)) {
                result = result | 0x20000;
            } else {
                result = result | 0x10000;
            }
        } else {
            dy = (rectA[3] + playerAWorldY) - (rectB[1] + playerBWorldY);
            if (playerB->moveState & MOVESTATE_GRAVITY_SWITCHED) {
                result = result | 0x10000;
            } else {
                result = result | 0x20000;
        ...
        if (ABS(dx) < ABS(dy)) {
            result = (result & 0xC0000) | (((dx << 8) & (byteMaskFF00 = 0xFF00)) | (dy & 0xFF));
        } else {
            result = (result & 0x30000) | (((dx << 8) & (byteMaskFF00 = 0xFF00)) | (dy & 0xFF));
        }

        if (0xC0000 & result) {
#ifndef NON_MATCHING
            register s32 var_r0 asm("r0") = result;
#else
            var_r0 = result;
#endif
            var_r0 &= byteMaskFF00;
...
void Call__Player_8014550(Player *p) { Player_8014550(p); }
