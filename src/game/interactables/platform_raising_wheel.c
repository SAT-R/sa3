...
typedef struct {
    ...
    /* 0x3C */ s32 qWorldX32;
    /* 0x40 */ s32 qWorldY32;
} PlatformRaisingWheel; /* size: 0x44 */

void Task_PlatformRaisingWheel(void);
void sub_8036304(void);
void sub_80363FC(Sprite *s);
void TaskDestructor_PlatformRaisingWheel(struct Task *);

void CreateEntity_PlatformRaisingWheel(MapEntity *me, u16 regionX, u16 regionY, u8 id)
{
    struct Task *t = TaskCreate(Task_PlatformRaisingWheel, sizeof(PlatformRaisingWheel),
                                0x2100, 0, TaskDestructor_PlatformRaisingWheel);
    ...
    wheel->worldX = worldX = TO_WORLD_POS(me->x, regionX);
    wheel->qWorldX32 = worldX;
    wheel->worldY = worldY = TO_WORLD_POS(me->y, regionY);
    wheel->qWorldY32 = worldY;

    s->x = wheel->qWorldX32 - gCamera.x;
    s->y = wheel->qWorldY32 - gCamera.y;
...
void Task_PlatformRaisingWheel(void)
{
    ...
    qWorldX = Q(wheel->worldX);
    qWorldY = Q(wheel->worldY);

    qTop = Q(me->d.sData[1] * TILE_WIDTH);
    qLeft = Q(me->d.sData[0] * TILE_WIDTH);

    qWorldX32 = wheel->qWorldX32;
    qWorldY32 = wheel->qWorldY32;

    qLeft = (qLeft * gStageData.unk5E[wheel->unk34]) >> 10;
    qLeft += qWorldX;
    wheel->qWorldX32 = qLeft;

    qTop = (qTop * gStageData.unk5E[wheel->unk34]) >> 10;
    qTop += qWorldY;
    wheel->qWorldY32 = qTop;
    ...
    for (i = 0; i < NUM_SINGLE_PLAYER_CHARS; i++) {
        ...
        if (((p->charFlags.someIndex == 1) || (p->charFlags.someIndex == 2)
            ...
            res = sub_8020950(s, I(wheel->qWorldX32), I(wheel->qWorldY32), p, NULL);
...
void sub_8036304(void)
{
    ...
    worldX = I(wheel->qWorldX32);
    worldY = I(wheel->qWorldY32);

    if (!IsPointInScreenRect(worldX, worldY)
        && !IsPointInScreenRect(wheel->worldX, wheel->worldY)) {
        for (i = 0; i < NUM_SINGLE_PLAYER_CHARS; i++) {
            // _08036350
            if (i != 0) {
                p = &gPlayers[p->charFlags.partnerIndex];
