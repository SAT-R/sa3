...
struct SaveGame
{
    ...
    /* 0x11 */ u8 unlockedZones; // 0x11 | Up to the index of this value all Zones are accessible (1 to 9)
    /* 0x12 */ u8 continueZone;  // 0x12 | Zone the player continues at on startup after character select (0 to 6) 
    /* 0x13 */ u8 unk13;
    ...
    /* 0x37 */ u8 collectedMedals[9][ACTS_PER_ZONE]; // 0x37 | (struct MedalCollection) xxxxxGSBb Medals collected for completion time in each [Zone][Act]
    /* 0x5B */ u8 unk5B;
    /* 0x5C */ u8 unk5C;
    /* 0x5D */ u8 unk5D;
